...
class App extends React.Component {
  ...
  componentDidMount(){
    ...
    fetch('https://dog.ceo/api/breeds/list/all').then((res) => {
      ...
      console.log(Object.keys(json.message)[Math.floor(Math.random()*this.state.breeds.length)]);
        this.setState({
    ...
    this.setState((currentState)=>{
      return{
        breed: this.state.breeds[Math.floor(Math.random()*this.state.breeds.length)]
      };
    });
    });
